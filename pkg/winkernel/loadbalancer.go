...
func (proxier *Proxier) manageLoadbalancer(lbConfig *loadbalancerConfig) (success bool) {
	...
	if !proxier.requiresUpdateLoadbalancer(lbConfig.hnsID, len(lbConfig.endpoints)) {
		proxier.deleteExistingLoadBalancer(proxier.hns, &lbConfig.hnsID, lbConfig.vip, lbConfig.protocol, lbConfig.internalPort, lbConfig.externalPort, lbConfig.endpoints, lbConfig.queriedLoadBalancers)
